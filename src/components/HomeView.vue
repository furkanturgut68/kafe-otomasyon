      ...
      <div class="grid">
        <!-- Salon Alan -->
          <div class="col-3 xl:col-1 bg-primary h-screen p-0 flex flex-column justify-content-center align-items-center">
            <div class="ml-1 col-10 hover:bg-white hover:text-900 p-0 mb-4 cursor-pointer" v-for="saloon in saloonList" :key="saloon.id" @click="selectSaloon(saloon.id)">
        ...
        <!-- Ana Alan -->
          <div class="col-9 xl:col-11 mt-4">
            <div class="flex flex-wrap">
              <div class="col-2 xl:col-1 bg-primary text-center flex justify-content-center  mb-2 mr-2 p-5  cursor-pointer" @click="selectTable(a, tableInfo[0].name)" v-for="a in tableSize" :key="a">
                  {{tableInfo[0].name}} {{a}}
              </div>
      ...
      await getDocs(q).then((querySnapshot) => {
        querySnapshot.forEach((payload) => {
          ...
          tableSize.value = payload.data().table;
            ...
            console.log("Info:", tableInfo.value);
        });
      }).catch(() => {
